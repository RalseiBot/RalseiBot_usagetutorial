# Ralsei Bot_使用教程

社区(都是官方的):
[![Discord](https://img.shields.io/discord/1305535111697141790?label=Discord&logo=discord&color=7289DA)](https://discord.gg/fBRMhdnhSW)

以后去看教程去: https://ralseibot.github.io (以后更新都在哪里)

繁体中文使用的翻译软件 如果发现翻译问题可以申请合并

繁體中文使用的翻譯軟體 如果發現翻譯問題可以申請合併

[简体中文](#简体中文教程) | [繁体中文](#繁体中文教程)

<a name="简体中文教程"></a>

注意:Ralsei Bot是在Astrbot上运行的

Ralsei Bot使用教程

1:如何让Bot回答你 @bot 发送 ! 均可触发bot回复

2:漂流瓶功能

2.1:扔漂流瓶
使用 /扔漂流瓶 扔出漂流瓶 

如果扔出去的漂流瓶不能包含恐吓，色情，威胁等内容
如果因此导致对应的qq bot账号被封禁你将会受到严厉的惩罚
我们也会定期检查漂流瓶里面的内容(已经打开的) 如果查出发送此类内容 立刻剥夺bot使用权

2.2:捡漂流瓶
使用 /捡漂流瓶 捡起漂流瓶(如果没有漂流瓶则捡不了)

3:使用规范
不允许短时间发送大量内容给bot(太烧token了)

4:猪猪表情包

你可以在 任意聊天频道发送 以下图片展示内容:

[图片](https://cdn.discordapp.com/attachments/1533133807689924759/1533185235863208046/Screenshot_20260802_025021_org_mozilla_fenix_HomeActivity.jpg?ex=6a6f9185&is=6a6e4005&hm=06f1e891cfcfe0569481a6350eb976a3f2e11dee217d93e98b8b1ad565b293ed&) 
(图片是来自Discord CDN的 如果你无法查看图片)

[无法查看图片的](https://ibb.co/qFD324qy)

图片里面展示的都可以

如果包含以下这些 以下图片展示内容:

[图片](https://cdn.discordapp.com/attachments/1533133807689924759/1533186200486023400/Screenshot_20260802_025428_org_mozilla_fenix_HomeActivity.jpg?ex=6a6f926b&is=6a6e40eb&hm=70aecd6aadc4fe2041b1fbe46ad8c4c01bd9c57aa16d78bcf0cfeceb9de94845&)
(图片是来着Discord CDN的 如果你无法查看图片)

[无法查看图片的](https://ibb.co/s9yB5J8F)

则不会触发猪猪表情包的发送

bot指令菜单:[指令菜单](https://cdn.discordapp.com/attachments/1322394053450535003/1533141997446234132/image.webp?ex=6a6f6940&is=6a6e17c0&hm=5d8ccd16e3981c22255054818cea5f21fcfbf1d2e468568eb27d26d3300f10e4&)
如果需要自行查看在任意频道发送 /helps即可

注意:最终解释权归Project BetterMine(Owner:wangzic and ShiroDye_awa 所属Bot:Ralsei Bot和Susie Bot)所有

在本仓库和官网的仓库发出的对Ralsei Bot的 Issues 和 Pull 一律不受理 请在官方Discord回报Ralsei Bot的错误

[回报错误频道(需加入BM Discord)](https://discord.com/channels/1305535111697141790/1533195904344002701)

错误将会尽快修复 如果是插件问题对应插件会被禁用(直到插件开发者修复)

[Bot更改频道(需加入BM Discord)](https://discord.com/channels/1305535111697141790/1533196235589156864)

目前已知问题:

Discord无法输入反斜杠导致无法无视设定(也就是人设) (也不可能修复吧)


<a name="繁体中文教程"></a>

注意:Ralsei Bot是在Astrbot上運行的

 Ralsei Bot使用教學課程

 1:如何讓Bot回答你 @bot 發送 ! 均可觸發bot回复

 2:漂流瓶功能

 2.1:丟漂流瓶
 使用 /丟漂流瓶 丟出漂流瓶

 如果丟出去的漂流瓶不能包含恐嚇，色情，威脅等內容
 如果因此導致對應的qq bot帳號被封鎖你將會受到嚴厲的懲罰
 我們也會定期檢查漂流瓶裡面的內容(已經打開的) 如果查出發送此類內容 立刻剝奪bot使用權

 2.2:撿漂流瓶
 使用 /捡漂流瓶 撿起漂流瓶(如果沒有漂流瓶則撿不了)

 3:使用規範
 不允許短時間發送大量內容給bot(太燒token了)

 4:豬豬表情包

 你可以在 任意聊天頻道發送 以下圖片展示內容:

[圖片](https://cdn.discordapp.com/attachments/1533133807689924759/1533185235863208046/Screenshot_20260802_025021_org_mozilla_fenix_HomeActivity.jpg?ex=6a6f9185&is=6a6e4005&hm=06f1e891cfcfe0569481a6350eb976a3f2e11dee217d93e98b8b1ad565b293ed&)
 (圖片是來自Discord CDN的 如果你無法查看圖片)

 [無法查看圖片的](https://ibb.co/qFD324qy)

 圖片裡面展示的都可以

 如果包含以下這些 以下圖片展示內容:

[圖片](https://cdn.discordapp.com/attachments/1533133807689924759/1533186200486023400/Screenshot_20260802_025428_org_mozilla_fenix_HomeActivity.jpg?ex=6a6f926b&is=6a6e40eb&hm=70aecd6aadc4fe2041b1fbe46ad8c4c01bd9c57aa16d78bcf0cfeceb9de94845&)
 (圖片是來著Discord CDN的 如果你無法查看圖片)

 [無法查看圖片的](https://ibb.co/s9yB5J8F)

 則不會觸發豬豬表情包的發送

 bot指令選單:[指令選單](https://cdn.discordapp.com/attachments/1322394053450535003/1533141997446234132/image.webp?ex=6a6f6940&is=6a6e17c0&hm=5d8ccd16e3981c22255054818cea5f21fcfbf1d2e468568eb27d26d3300f10e4&)
 如果需要自行查看在任意頻道發送 /helps即可

 注意:最終解釋權歸Project BetterMine(Owner:wangzic and ShiroDye_awa 所屬Bot:Ralsei Bot和Susie Bot)所有

 在本倉庫和官網的倉庫發出的對Ralsei Bot的 Issues 和 Pull 一律不受理 請在官方Discord回報Ralsei Bot的錯誤

 [回報錯誤頻道(需加入BM Discord)](https://discord.com/channels/1305535111697141790/1533195904344002701)

 錯誤將會盡快修復 如果是插件問題對應插件會被停用(直到插件開發者修復)

 [Bot更改頻道(需加入BM Discord)](https://discord.com/channels/1305535111697141790/1533196235589156864)

 目前已知問題:

 Discord無法輸入反斜線導致無法無視設定(也就是人設) (也不可能修復吧)

由於命令關係 導致指令必須是簡體中文才有回應

可以透過複製來解決
